...
CATEGORIES["all"] = CATEGORIES["almost_all"] + CATEGORIES["remaining"]
...
def check_list_arg(arg, name, list):
    """Check that the arg is either an element or a subset of the list,
    or the string 'all', returns a singleton, sub-list or whole list
    respectively"""
    if isinstance(arg, str):
        if arg == "all":
            return list
        arg = [arg]
    err_msg = f"{name} arg must be 'all' or an element or a subset of {list}"
    assert set(arg).issubset(list), err_msg
    return arg
